...
import { InstructionTool } from "../ins/tool";
...
export class PlcTcpProxy {
  ...
  public constructor(name: string, host: string, port: number) {
    this.client = new TCPClient(name, { host, port }, async ({ data }) => {
      ...
      const len = InstructionTool.loHiToNum(dataArr[2], dataArr[3]);
      ...
      if (isSync(receiveModuleNum, receiveNum)) {
        ...
        if (this.getLatestSendNo() === receiveSendNo) this.getResolve()(data);
      } else {
        // plc to server
  ...
  public setPlcReportDataHandler(handler) {
  ...
  private async _send(instruction: Instruction) {
    console.log(
      "发送HEX数据:",
      chalk.yellow(InstructionTool.arrToHexStr(instruction.toArr()))
    );
  ...
  private _logAfterSend(resultArr: number[], startTime: dayjs.Dayjs) {
    ...
    console.log(
      `接收Hex数据: ${chalk.magentaBright(
        InstructionTool.arrToHexStr(resultArr)
      )}`
    );
    const endTime = dayjs();
